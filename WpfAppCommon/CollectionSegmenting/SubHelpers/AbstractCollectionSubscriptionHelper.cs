...
namespace WpfAppCommon.CollectionSegmenting.SubHelpers
{
    ...
    public abstract class AbstractCollectionSubscriptionHelper<TItem> : IDisposable
    {
        ...
        private readonly ObservableCollection<TItem> savedItems = new ObservableCollection<TItem>();
        ...
        private void AddItem(TItem item, int index, bool initial)
        {
            ...
            {
                throw new ArgumentOutOfRangeException("index", GetType().Name + " has become inconsistent: tried to insert an item at the index " + index + " when the cached collection length was " + savedItems.Count);
            }
        ...
        private void MoveItem(TItem item, int oldIndex, int newIndex)
        {
            ...
            {
                throw new ArgumentOutOfRangeException("oldIndex", GetType().Name + " has become inconsistent: tried to move an item from an index " + oldIndex + " while the length of the cached collection was " + savedItems.Count);
            }
            if (newIndex < 0 || newIndex >= savedItems.Count)
            {
                throw new ArgumentOutOfRangeException("newIndex", GetType().Name + " has become inconsistent: tried to move an item to an index " + newIndex + " while the length of the cached collection was " + savedItems.Count);
            }
        ...
        private void RemoveItem(TItem item, int index)
        {
            ...
            if (index < 0 || index >= savedItems.Count)
            {
                throw new ArgumentOutOfRangeException("index", GetType().Name + " has become inconsistent: tried to remove an item at the index " + index + " when the cached collection length was " + savedItems.Count);
            }
