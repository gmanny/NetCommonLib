...
namespace WpfAppCommon.CollectionSegmenting
{
    ...
    public class ItemBlockHandler<TOuterItem, TInnerItem> : IDisposable
    {
        ...
        private readonly Stack<TInnerItem> itemAdditionStack = new Stack<TInnerItem>();
        private readonly HashSet<TOuterItem> itemSet = new HashSet<TOuterItem>();
        ...
        public ItemBlockHandler([NotNull] IItemBlock<TInnerItem> block, [NotNull] ObservableCollection<TOuterItem> outerCollection, [NotNull] Func<TInnerItem, TOuterItem> itemConverter, [NotNull] IItemBlockParent<TOuterItem, TInnerItem> itemBlockParent, int initialCollectionIndex, Action<ItemBlockHandler<TOuterItem, TInnerItem>> beforeInsertion)
        {
            ...
            if (outerCollection == null) throw new ArgumentNullException("outerCollection");
            if (itemConverter == null) throw new ArgumentNullException("itemConverter");
            if (itemBlockParent == null) throw new ArgumentNullException("itemBlockParent");
            if (initialCollectionIndex < 0 || initialCollectionIndex > outerCollection.Count)
            {
                throw new ArgumentOutOfRangeException("initialCollectionIndex",
        ...
        private void RemoveItem(TInnerItem item, int index)
        {
            ...
            {
                throw new InvalidOperationException(String.Format("Found another item at the specified removal index (block index: {0}, removal index: {1})", collectionIndex, index));
            }
        ...
        /// </summary>
        public IItemBlock<TInnerItem> Block
        {
            get { return block; }
        }
        ...
        public bool HasItem(TOuterItem item)
        {
