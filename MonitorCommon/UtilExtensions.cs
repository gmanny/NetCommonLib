...
namespace MonitorCommon
{
    ...
    public static class UtilExtensions
    {
        ...
        public static void ForEach<T>(this Nullable<T> c, Action<T> action) where T : struct
        {
            if (c.HasValue)
            {
                action(c.Value);
        ...
        public static IEnumerable<int> Indicies(this ICollection c) => Enumerable.Range(0, c.Count);
        ...
        public static string Truncate2(this string value, int maxLength)
        {
            if (!string.IsNullOrEmpty(value) && value.Length > maxLength)
            {
                return value.Substring(0, maxLength / 2 - 1) + "..." + value.Substring(value.Length - (maxLength / 2 - 1));
            }
