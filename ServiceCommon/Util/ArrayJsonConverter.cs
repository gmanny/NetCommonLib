        ...
        public sealed override T ReadJson(JsonReader reader, Type objectType, T existingValue, bool hasExistingValue, JsonSerializer serializer)
        {
            if (reader.TokenType != JsonToken.StartArray)
            {
                throw new JsonException(
                    $"{nameof(T)} is represented by an array, but got {reader.TokenType} token");
            }
            ...
            {
                throw new JsonException(
                    $"{nameof(T)} array representation should contain {ArrayElementCount} items, but got {item.Count}: {item}");
            }
